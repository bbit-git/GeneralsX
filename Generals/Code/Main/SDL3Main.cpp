...
#if defined(__ANDROID__)
		// GeneralsX @feature Android data root. The launcher Activity exports
		// CNC_GENERALS_PATH (its files/generals dir, where the .BIG archives are
		// adb-pushed). registry.cpp reads that env var, but most of the engine's
		// file lookups are relative to CWD with a case-insensitive fallback, so
		// also chdir() there. App-private storage needs no runtime permission.
		{
			const char* dataDir = getenv("CNC_GENERALS_PATH");
			if (dataDir && *dataDir) {
				if (chdir(dataDir) == 0) {
